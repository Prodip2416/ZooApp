...
namespace ZooApp.Web.Mvc.Controllers
{
    public class AnimalFoodsController : Controller
    {
        ...
        {
            var results = service.ViewFoodTotalsDetails();

            ViewBag.Total = results.Sum(x => x.TotalPrice);
            return View(results);
        }
        public ActionResult Details(int id)
        {
            var foodDetails = service.ViewFoodTotalsDetailsByFoodId(id);
            ViewBag.Total = foodDetails.Sum(x => x.TotalPrice);
